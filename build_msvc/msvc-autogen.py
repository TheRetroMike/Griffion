...
def parse_config_into_griff_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"Griffion Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/griffion_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/griffion_config.h'), "w", encoding="utf8") as griff_config:
        griff_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='Griffion-core msbuild configuration initialiser.')
    ...
    parse_config_into_griff_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/griffion_config.h'), os.path.join(SOURCE_DIR, 'config/griffion-config.h'))
