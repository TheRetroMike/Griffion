...
"""Script for verifying Griffion Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from griffion.org and griffion.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/griffion-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://griffion.org"
HOST2 = "https://griffion.org"
VERSIONPREFIX = "griffion-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the Griffion community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"griffion_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by griffion.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the griffion release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes griffion.org lags behind griffion.org.)')
    )
