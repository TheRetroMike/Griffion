# Bootstrappable Griffion Core Builds

This directory contains the files necessary to perform bootstrappable Griffion
Core builds.
...
To build Griffion Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Griffion Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/griffion-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/griffion-core/guix.sigs).
