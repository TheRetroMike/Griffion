...
""" Example logging Griffion Core mempool events using the mempool:added,
...
def main(griffiond_path):
    griffiond_with_usdts = USDT(path=str(griffiond_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    griffiond_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    griffiond_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    griffiond_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    griffiond_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[griffiond_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/griffiond")
        exit(1)
