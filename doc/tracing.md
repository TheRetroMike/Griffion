# User-space, Statically Defined Tracing (USDT) for Griffion Core

Griffion Core includes statically defined tracepoints to allow for more
...
**Note**: Griffion Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to Griffion Core
...
Multiple tools can list the available tracepoints in a `griffiond` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in Griffion Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/griffiond
stap net        outbound_message 0x0000000000107c05 /src/griffiond
stap validation block_connected  0x00000000002fb10c /src/griffiond
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in Griffion Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/griffiond | grep NT_STAPSDT -A 4 -B 2
