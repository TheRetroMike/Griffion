# CJDNS support in Griffion Core

It is possible to run Griffion Core over CJDNS, an encrypted IPv6 network that
...
done outside of Griffion Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the Griffion network and individual nodes.
...
before it will work with your Griffion Core node. This is described in steps
...
## Run Griffion Core with CJDNS

Once you are connected to the CJDNS network, the following Griffion Core
...
When enabled, this option tells Griffion Core that it is running in an
...
IPv6 local network. This helps Griffion Core perform better address management:
...
CJDNS support was added to Griffion Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `griffion-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/GriffionProject/Griffion/blob/master/doc/tor.md) for
details.

## CJDNS-related information in Griffion Core

There are several ways to see your CJDNS address in Griffion Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `griffion-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `griffion-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `griffion-cli help getnodeaddresses` for details.
