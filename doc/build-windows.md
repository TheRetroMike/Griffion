...
Below are some notes on how to build Griffion Core for Windows.

The options known to work for building Griffion Core on Windows are:
...
Note that for WSL the Griffion Core source path MUST be somewhere in the default mount file system, for
example /usr/src/griffion, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\griffion`, for example:

    make install DESTDIR=/mnt/c/workspace/griffion
