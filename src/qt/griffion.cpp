...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<GriffionUnit>("GriffionUnit");
#else
    qRegisterMetaType<GriffionUnit>("GriffionUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. griffion_de.qm (shortcut "de" needs to be defined in griffion.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. griffion_de_DE.qm (shortcut "de_DE" needs to be defined in griffion.qrc)
...
static const char* qt_argv = "griffion-qt";

GriffionApplication::GriffionApplication()
...
void GriffionApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the GriffionApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", GriffionGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void GriffionApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool GriffionApplication::createOptionsModel(bool resetSettings)
{
...
void GriffionApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new GriffionGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &GriffionGUI::quitRequested, this, &GriffionApplication::requestShutdown);

    window->setMinimumSize(1124,700);
    window->setBaseSize(1124,700);
...
void GriffionApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void GriffionApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool GriffionApplication::baseInitialize()
{
    return node().baseInitialize();
}

void GriffionApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &GriffionApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &GriffionApplication::handleRunawayException);
    connect(this, &GriffionApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &GriffionApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void GriffionApplication::parameterSetup()
{
...
void GriffionApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void GriffionApplication::requestInitialize()
{
...
void GriffionApplication::requestShutdown()
{
...
void GriffionApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // griffion: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &GriffionGUI::handlePaymentRequest);
            connect(window, &GriffionGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void GriffionApplication::handleRunawayException(const QString &message)
{
...
void GriffionApplication::handleNonFatalException(const QString& message)
{
...
WId GriffionApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool GriffionApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", GriffionGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(griffion);
    Q_INIT_RESOURCE(griffion_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(GRIFFION_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see griffion-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, PACKAGE_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see griffion-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse griffion.conf, determine network, switch to network specific
    ...
    // griffion: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
