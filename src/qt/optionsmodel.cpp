...
/** Call node.updateRwSetting() with Griffion 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because Griffion 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/GriffionProject/Griffion/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because griffion 22.x will not try to read these.
...
//! Convert enabled/size values to griffion -prune setting.
...
//! Get pruning enabled value to show in GUI from griffion -prune setting.
...
//! Get pruning size value to show in GUI from griffion -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayGriffionUnit")) {
        settings.setValue("DisplayGriffionUnit", QVariant::fromValue(GriffionUnit::GRIFF));
    }
    QVariant unit = settings.value("DisplayGriffionUnit");
    if (unit.canConvert<GriffionUnit>()) {
        m_display_griffion_unit = unit.value<GriffionUnit>();
    } else {
        m_display_griffion_unit = GriffionUnit::GRIFF;
        settings.setValue("DisplayGriffionUnit", QVariant::fromValue(m_display_griffion_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if griffion.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_griffion_unit);
...
{
    if (new_unit.isNull() || new_unit.value<GriffionUnit>() == m_display_griffion_unit) return;
    m_display_griffion_unit = new_unit.value<GriffionUnit>();
    QSettings settings;
    settings.setValue("DisplayGriffionUnit", QVariant::fromValue(m_display_griffion_unit));
    Q_EMIT displayUnitChanged(m_display_griffion_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/GriffionProject/Griffion/pull/8273
    ...
    // (https://github.com/griffion-core/gui/issues/567).
