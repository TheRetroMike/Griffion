...
#include <qt/griffionaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Griffion address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new GriffionAddressEntryValidator(parent));
    widget->setCheckValidator(new GriffionAddressCheckValidator(parent));
}
...
bool parseGriffionURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no griffion: URI
    if(!uri.isValid() || uri.scheme() != QString("griffion"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!GriffionUnits::parse(GriffionUnit::GRIFF, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseGriffionURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseGriffionURI(uriInstance, out);
}

QString formatGriffionURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("griffion:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(GriffionUnits::format(GriffionUnit::GRIFF, info.amount, false, GriffionUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openGriffionConf()
{
    ...
    /* Open griffion.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Griffion.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Griffion (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("Griffion (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for Griffion*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "griffion.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("griffion-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a griffion.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=Griffion\n";
        else
            optionFile << strprintf("Name=Griffion (%s)\n", ChainTypeToString(chain));
