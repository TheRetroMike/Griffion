...
const char * const GRIFFION_CONF_FILENAME = "griffion.conf";
const char * const GRIFFION_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //griffion-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", GRIFFION_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Griffion
    // macOS: ~/Library/Application Support/Griffion
    // Unix-like: ~/.griffion
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Griffion";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Griffion";
#else
    // Unix-like
    return pathRet / ".griffion";
#endif
#endif
}
