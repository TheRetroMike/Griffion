...
"""Griffion test framework primitive and message structures
...
COIN = 100000000  # 1 griff in satoshis
MAX_MONEY = 11000000000 * COIN
...
MAGIC_BYTES = {
    "mainnet": b"\x87\xaa\xd7\xbc",   # mainnet
    "testnet3": b"\xc6\xda\xfa\xe8",  # testnet3
    "regtest": b"\xec\xdd\xdf0\xc1",   # regtest
}
...
# Objects that map to griffiond objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/griffion/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
    NET_IPV6 = 2
    NET_TORV3 = 4
    NET_I2P = 5
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += (0).to_bytes(4, "little", signed=True)  # Griffion Core ignores the version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in griffiond
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
    ...
    def is_valid(self):
        ...
        for tout in self.vout:
            if tout.nValue < 0 or tout.nValue > 11000000000 * COIN:
                return False
        return True
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in griffiond
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in griffiond indicates these should be deserialized as blocks
