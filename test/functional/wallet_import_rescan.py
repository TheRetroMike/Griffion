...
import RPC call and sends GRIFF to it. Then other nodes import the addresses,
and the test makes listtransactions and getbalance calls to confirm that the
importing node either did or did not execute rescans picking up the send
transactions.

In the second part of the test, node 0 sends more GRIFF to each address, and the
...
class ImportRescanTest(GriffionTestFramework):
    ...
    def run_test(self):
        ...
        for variant in mempool_variants:
            ...
            child = self.nodes[1].send(
                ...
                outputs=[{ADDRESS_TRF1_UNSPENDABLE : variant.initial_amount}],
