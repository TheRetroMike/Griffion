...
"""Test framework for griffion utils.
...
def main():
    ...
    bctester(os.path.join(env_conf["SRCDIR"], "test", "util", "data"), "griffion-util-test.json", env_conf)
...
def bctest(testDir, testObj, buildenv):
    ...
    if testObj["exec"] == "./griffion-util":
        execprog = os.getenv("GRIFFIONUTIL", default=execprog)
    elif testObj["exec"] == "./griffion-tx":
        execprog = os.getenv("GRIFFIONTX", default=execprog)
    ...
    if "error_txt" in testObj:
        ...
        # That stderr is empty if no errors are expected. However, griffion-tx
